...
def get_all_solicitudes(
    ...
    return [{
        ...
        "id_negocio": str(s.id_negocio),
        "id_asesor": str(s.id_asesor) if s.id_asesor else None,
        "id_producto_credito": str(s.id_producto_credito),
        "canal_origen": s.canal_origen,
        "monto_solicitado": float(s.monto_solicitado),
        "monto_aprobado": float(s.monto_aprobado) if s.monto_aprobado is not None else None,
        "plazo_meses": s.plazo_meses,
        "moneda": s.moneda,
        "tea_referencial": float(s.tea_referencial),
        "con_seguro_desgravamen": s.con_seguro_desgravamen,
        "garantia": s.garantia,
        "destino_credito": s.destino_credito,
        "cuota_estimada": float(s.cuota_estimada),
        "estado": s.estado,
...
def get_cobranza(
    ...
    overdue_items = db.query(
        ...
        Cliente.nombres.label("cliente_nombres"),
        Cliente.apellidos.label("cliente_apellidos")
    ).join(Credito, CronogramaPago.id_credito == Credito.id_credito)\
     .join(Cliente, Credito.id_cliente == Cliente.id_cliente)\
     .filter(
         CronogramaPago.estado.in_(["PENDIENTE", "VENCIDA", "PARCIAL"]),
    ...
    for item in overdue_items:
        cp = item[0]
        mora_days = (today - cp.fecha_pago).days
        unpaid_amount = cp.monto_cuota - cp.monto_pagado
        urgency = "Rojo" if mora_days > 90 else ("Naranja" if mora_days > 30 else "Amarillo")
        results.append({
            "id_cobranza": str(cp.id_cuota),
            "cliente": f"{item.cliente_nombres} {item.cliente_apellidos}",
            "dias_mora": mora_days,
            "monto_vencido": float(unpaid_amount),
            "urgencia": urgency
        })
    return results
