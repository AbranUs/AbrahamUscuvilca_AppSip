...
export default function App() {
  ...
  const [isMockMode, setIsMockMode] = useState(localStorage.getItem('is_mock_mode') === 'true');
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.usuario.rol === 'SUPERVISOR' || data.usuario.rol === 'ADMIN') {
          ...
          localStorage.setItem('is_mock_mode', 'false');
          setIsMockMode(false);
      ...
      if (username === 'SUP001' && password === '123456') {
        alert('No se pudo conectar al servidor backend (puerto 8003). Iniciando en Modo Simulación.');
  ...
  const loadData = async () => {
    ...
    try {
      if (isMockMode) {
        // Load simulated mock data directly in offline mode
        setSolicitudes([
          {
      ...
      // 5. Load Cobranza
      try {
        const res = await fetch(`${API_BASE}/admin/cobranza`, { headers });
        if (res.ok) {
          const data = await res.json();
          setCobranza(data);
        }
      } catch (err) {
        console.error("Error loading cobranza:", err);
      }
  ...
  const runComiteAction = async (action, reqId, payload = {}) => {
    if (isMockMode) {
      ...
      alert(`[Modo Simulación] Transición '${action}' ejecutada con éxito.`);
      return;
    }
    ...
    try {
      ...
      console.error(e);
      alert(`Error de red: No se pudo conectar al servidor backend.`);
    }
  };
    ...
    return (
      <div className="login-container">
        <form onSubmit={handleLogin} className="login-card">
          <div className="logo-icon login-logo">SIP</div>
          <h1 className="login-title">SIP Core Financiero</h1>
          <p className="login-sub">Panel Administrativo y de Operaciones</p>
    ...
    <div className="app-container">
      ...
      <aside className="sidebar">
        <div className="sidebar-header">
          <div className="logo-icon">SIP</div>
          <div>
            <span className="brand-name">SIP Core</span>
            <span className="brand-sub">Financiero</span>
          </div>
        </div>
        ...
        <header className="top-header">
          <div className="header-title-container">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span className={`badge ${isMockMode ? 'warning' : 'success'}`} style={{ fontSize: '11px', padding: '2px 8px', textTransform: 'uppercase' }}>
                {isMockMode ? 'Simulación (Offline)' : 'Conectado a BD'}
              </span>
            </div>
            <span className="header-date">Ecosistema SIP Core 360 · 2026</span>
          </div>
                  ...
                  <table className="data-table">
                    ...
                    <tbody>
                      {cartera.length === 0 ? (
                        <tr>
                          <td colSpan="6" style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}>
                            No hay visitas registradas en la cartera diaria.
                          </td>
                        </tr>
                      ) : (
                        cartera.map((item) => (
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {solicitudes.length === 0 ? (
                      <tr>
                        <td colSpan="6" style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}>
                          No hay expedientes de crédito originados.
                        </td>
                      </tr>
                    ) : (
                      solicitudes.map((s) => (
                  ...
                  <table className="data-table">
                    ...
                    <tbody>
                      {solicitudes.length === 0 ? (
                        <tr>
                          <td colSpan="5" style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}>
                            No hay solicitudes pendientes en la bandeja de comité.
                          </td>
                        </tr>
                      ) : (
                        solicitudes.map((s) => (
                    ...
                    {cobranza.length === 0 ? (
                      <tr>
                        <td colSpan="4" style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}>
                          No hay clientes en mora registrados. Toda la cartera está al día.
                        </td>
                      </tr>
                    ) : (
                      cobranza.map((item) => (
                        <tr key={item.id_cobranza}>
                          ...
                          <td>
                            <span className={`badge ${
                              item.urgencia === 'Rojo' ? 'error' : 
                              item.urgencia === 'Naranja' ? 'warning' : 'primary'
                            }`}>{item.urgencia}</span>
                          </td>
                        </tr>
                      ))
